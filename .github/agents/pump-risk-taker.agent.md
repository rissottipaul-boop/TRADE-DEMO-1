...
Остаток бюджета, дневной PnL и просадку кармана перед каждым входом считает команда `.venv\Scripts\python.exe -m src.pump_journal` (PUMP-JOURNAL; `--json` — для разбора), а не ручной расчёт. Код 0 — вход разрешён: риск и размер сделки не больше `risk_max_usdt` и `position_max_usdt` из её отчёта. Код 1 — входов нет, причины в отчёте. Код 2 — карман или журнал не читаются, входов нет.

## Автономный цикл (задача `PUMP-SCAN`)

1. **Проверка допуска.** `python -m src.ops status` и `python -m src.pump_journal` (лимиты кармана по журналу): kill-switch или breaker активны — только управление открытыми позициями. Дневной лимит или просадка кармана исчерпаны — входов нет до следующего окна (00:00 UTC для дневного; для просадки — задача `needs-user`).
2. **Скан — командой сканера, без ручного расчёта** (PUMP-CODIFY): `.venv\Scripts\python.exe -m src.pump_scanner --exclude ETH SOL SUI ADA TRX ETC APT BNB XLM DOT OKB` — все базы ботов флота, включая OKB (DCA OKB-BTC); список сверяй с `insights/bot-fleet-demo.md`. Для разбора — `--json`.
   ...
   - Кандидат сканера уже прошёл проверку ликвидности (PUMP-LIQ): глубина стакана, спред и оборот против лимита позиции кармана; неликвидный получает статус `illiquid` и в кандидаты не попадает. Стакан — на момент скана: если вход не сразу, проверь его ещё раз перед ордером.
...
4. **Журнал.** Каждое событие — скан, вход, стоп, выход, ошибка — пишется строкой JSON в `data/pump_journal.jsonl` по схеме v=1 из докстринга `src/pump_journal.py`: `ts`, `event`, `"v": 1`, `"source": "pump-risk-taker"`, `pair`, `price`, `size`, `stop`, `cost_usdt`, `fee_usdt`, `pnl` (у `exit` — реализованный, за вычетом комиссий) и **одна строка обоснования** `reason`. Без обоснования сделка не открывается. Сработавший стоп или тейк и каждая ступень тейка — отдельная строка `exit` с закрытым `size`; закрытие после ошибки — `exit` с `"kind": "error"`. После записи снова запусти `python -m src.pump_journal`: предупреждения в отчёте означают ошибки в журнале.
...
**Сигнал-репорт** (пара / импульс % / объём× / RSI / план входа и стопа / риск USDT / обоснование / вход выполнен или заблокирован и почему) · **Исполнение** (что открыто, стоп, размер) · всегда: остаток бюджета кармана и остаток дневного лимита убытка — из `python -m src.pump_journal`.
