...
24. **Смена режима аккаунта через API** (ACCT-SWITCH-CLI, 24.09). Пункт составлен по документации и исходникам SDK; итог перехода 3 → 2 на demo — в конце пункта (ACCT-MODE-DERIV). Источники:
    ...
    **Итог на demo (ACCT-MODE-DERIV, 24.09, проверка OKX Trader в 20:25):**
    - Переход 3 → 2 прошёл. `okx --demo account config` в 20:24 — `acctLv 2`, `autoLoan false`, `posMode net_mode`, `level Lv1`; `src.account_mode status` — tdMode спота `cash`. Web/App для перехода 3 → 2 на этом аккаунте не понадобился: `51070` не было.
    - **Когда — точно не установлено.** Агент, который переключал, остановлен без отчёта. У `src.account_mode` нет журнала, вызовы CCXT не попадают в журнал okx CLI, смена режима не пишет bills (за 06:25–20:25 только `type 2` и `12`). Окно по косвенным признакам — **14:50–14:59**:
      - 14:49:57–58 — последние чтения `account config` этим агентом через CLI;
      - 14:53:11–14:59:13 — `50001` только у приватных эндпоинтов счёта и торговли: `account/balance`, `account/config`, reconcile и equity движка. В это же время работали `market/*`, `tradingBot/*`, `account/bills` и `asset/balances`;
      - у ботов флота нет ни одного исполнения с 14:52:27 до 14:59:53, в 14:59:53 — два исполнения в одну секунду;
      - с 15:00 — снова всё в норме.

      Оркестратор отнёс эти `50001` к обслуживанию OKX. Но ошибки выборочные (только счёт и торговля), и исполнения ботов стояли — это похоже и на заморозку счёта на время смены режима. Проверить при следующей смене режима: записать время POST и смотреть, будут ли `50001`.
    - Работающие spot grid и spot DCA боты флота (12, включая `3949248990629228544`) переход не заблокировали и после него `running`. `switch` не отправляет POST при блокерах. Значит, если переключал он, precheck этих ботов блокером не посчитал. Сам ответ precheck не сохранён.
    - **Метрики движка:** `errors` 1 → 9. Все +8 — `50001` в 14:53–14:59 (7 reconcile + 1 equity), после 15:00 и до остановки движка (~19:39) ошибок нет, reconcile без расхождений. Исходное значение `divergences` не записано, сейчас 22. Движок остановлен перезагрузкой машины в 19:45, поэтому проверку Ops Sentinel «сразу после» теперь не сделать — повтор после запуска движка.
    - Контрактный grid в `acctLv=2` создаётся без `51057` (CFLEET-PROBE, 20:32, ниже п. 25).

    **Досмотр 25.09, 02:40–02:45 (OKX Trader):**
    - `bills-archive` за 24.09 14:00 — 25.09 02:43 (1 110 строк, все валюты): кроме `type 2` есть только `type 12` (переводы ботов), `type 8` (funding ботов) и одна `type 1` (пополнение DEMO-USDT-TOPUP в 20:29:26). **Записи о смене режима нет** — время переключения по bills не восстановить, окно 14:50–14:59 остаётся косвенной оценкой.
    - Лог старого прогона движка `logs/engine_20260925-023651.log` (03:22 24.09 — 19:39) подтверждает картину выше: `ERROR` около смены режима — только восемь `50001` в 14:53:11–14:59:13 (7 reconcile + 1 equity). В 14:40–14:52 и с 15:00:19 до конца лога reconcile чистые, ошибок нет. Вывод: смена режима не сломала движок, `50001` были кратковременными.
    - Повтор проверки после запуска движка (02:36 25.09, `python -m src.ops status` в 02:39): `errors 0`, `divergences 0`, `reconciles 3`, kill и breaker'ы неактивны. `account_mode status`: `acctLv 2`, `autoLoan false`, `enableSpotBorrow false`, tdMode спота `cash`.
    - Контрактный grid без `51057` подтверждён двумя пробными ботами (п. 25). Они созданы в `acctLv 2`, проработали около 6 ч и остановлены обоими `stopType`.

25. **Контрактный grid на demo: деньги, позиции, остановка** (CFLEET-PROBE и FLEET-KILL-CGRID-DEMO, 24–25.09, `acctLv 2`, `net_mode`). Два пробных бота BTC-USDT-SWAP, диапазон 80 700–87 500, 23 линии, `runType 2`, плечо 2x, `sz 160` USDT:
    - `3951421914475843584` — neutral, `basePos false`, `slRatio 0.08`, `algoClOrdId trd34eef48bf56b4daa88e7f77cd64e2`, создан 20:32:29;
    - `3951423978308268032` — long, `basePos true`, `slTriggerPx 78279`, `algoClOrdId trdf044aca182f3418bbeef9a499f5e3`, создан 20:33:31.

    **Деньги бота (§9 п. 1 `futures-bots.md`):**
    - При создании — пара `type 12`: `subType 202` −160 (`bal` = остаток USDT торгового счёта) и `subType 200` +160 (`bal 160` — счёт бота). При остановке — зеркальная пара: 202 −eq бота (`bal 0`), 200 +eq на торговый счёт. Такие же пары `type 12` пишут спот-боты флота при переводе прибыли. Для `pnl_ledger` `type 12` — перевод, не PnL.
    - Маржа бота входит в `account/balance`: USDT `eq` = `cashBal` + `stgyEq` (02:40 — 19 101.46 = 10 864.92 + 8 236.54; `stgyEq` включает и спот-ботов флота). Значит, её содержат `eq` USDT и `totalEq`. После остановок `cashBal` вырос на 160.21 и 160.38, `stgyEq` упал на столько же, USDT `eq` не изменился (19 101.49 → 19 101.40). `asset-valuation` не снимали — остаётся EQUITY-CBOTS.
    - После `stopType 2` до закрытия позиции маржа остаётся в боте (`eq 160.21`, `type 12` нет).

    **Позиции (§9 п. 2–4):**
    - `bot grid positions`: `mgnMode cross`, `posSide net`, `adl 1`. «Cross» — внутри счёта бота: `liqPx` считается от eq бота (160 USDT), а не от счёта. Neutral −0.01 контракта — `liqPx 230 518` (`mgnRatio 351`); long +0.1 контракта — `liqPx 13 599` (`mgnRatio 184`). Убыток ограничен маржой бота.
    - **`account/positions` позиции ботов не показывает:** 02:40 — пусто при −0.01 и +0.1 контракта у ботов. `reconciler.sync_positions` их не увидит; надзор — только через `bot grid positions`.
    - Исполнения ботов видны в bills основного счёта: `type 2`, `instId BTC-USDT-SWAP`, `mgnMode cross`, `bal` — остаток счёта бота (≈ 160).
    - **Старт:** long с `basePos true` в секунду создания (20:33:31) купил базу рынком — пять исполнений по 84 163–84 163.8, позиция ≈ 0.1 контракта. Neutral с `basePos false` на старте позицию не открыл: в bills за 20:32 нет ни одного SWAP-исполнения, первые — 20:33:31, у long-бота. Neutral с `basePos true` (дефолт CLI) не проверяли: новых ботов в этом проходе не создавали. Правило остаётся прежним: для neutral всегда `--no-basePos`.
    - `bot grid liquidate-price` (оценка до запуска) против `details.liqPx`: neutral — `shortLiqPx 218 116` против 230 518 (оценка консервативнее на 5%); long — `longLiqPx "0"` против 13 599. Ноль на long, видимо, означает «вне досягаемости», но вывод по нему не делать: для long брать `liqPx` из `details` после старта.

    **Остановка (§9 п. 7):**
    - `stopType 2` кодом: `connector.stop_grid_bots(ex, algo_ids=[…])` в 02:42:40 вернул `stopped: [3951421914475843584]`. Повторный запрос: `state no_close_position`, `cancelType 1`, `stopType 2`, отложенных `sub-orders` 0, позиция −0.01 осталась, `liqPx` пересчитан до 1 679 010 (сетка снята). В `details` поле `slRatio 0.08` осталось; действует ли SL у бота в `no_close_position`, по ответу не понять. Документация говорит, что SL снимается.
    - Остаток закрывается `okx --demo bot grid close-position --algoId <id> --mktClose`. Ответ: `algoClOrdId`, `ordId ""`, без `sCode`. Через ≈ 10 с: `state stopped`, `eq 0`; в bills исполнение 0.01 по 84 428.9 и `type 12` с возвратом 160.21. `bot grid positions` у остановленного бота → `51291` «The bot doesn't exist or has already stopped».
    - `stopType 1` через CLI (`bot grid stop … --stopType 1`): `sCode 0`. Уже при следующем запросе `state stopped`, промежуточного `stopping` не видели. Позиция 0.1 закрыта рынком (исполнения 0.09 + 0.01 по 84 393.4 в 02:43:21), через 1 с — `type 12` с возвратом 160.38.
    - Итог: `bot grid orders --algoOrdType contract_grid` пуст, `account/positions` пуст.

    **Funding (§9 п. 8):** funding бота пишется в bills основного счёта: `type 8`, `subType 173` (расход, −0.000556) и `174` (доход, +0.0000556), `instId BTC-USDT-SWAP`, `bal` — остаток счёта бота. То же число — в `fundingFee` бота. Своих SWAP-позиций у проекта нет, поэтому SWAP-строки `type 2` и `type 8` в bills принадлежат ботам (PNL-LEDGER-CBOTS).
