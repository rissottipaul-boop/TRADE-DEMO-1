...
# --- Выход из позиции (ROUTER-EXIT, 2026-09-24) ---
def register_entry_size(inst_id: str, side: str, sz) -> bool:
    """Размер входа для слота register_entry — верхняя граница позиции, с которой
    сверяется выход. Зовёт OrderRouter.place_order после register_entry. Лимиты,
    счётчики и heat не трогает. False — слота с этой стороной нет или размер
    некорректен (выход через роутер тогда отклоняется)."""

def open_position(inst_id: str) -> dict | None:
    """Слот risk_open_risk (side, risk_pct, opened_at) и остаток sz с pos_id из
    risk_position_size; sz=None — остаток неизвестен (вход без размера)."""

def check_exit_allowed(inst_id: str, side: str, sz=None) -> tuple[bool, str, dict | None]:
    """Допуск ВЫХОДА. Kill-switch, breaker'ы, свежесть equity, пауза, лимит входов,
    блокировка инструмента, хедж и heat НЕ проверяются. Требуется: позиция по
    inst_id есть, сторона противоположна, остаток известен, 0 < sz <= остатка
    (sz=None — весь остаток). Отказ — событие exit_denied."""

def release_position(inst_id: str, filled_sz, pos_id: str | None = None) -> dict:
    """Исполненный выход: остаток -= filled_sz, у нуля слот и heat освобождаются.
    Позиция сменилась (pos_id), остаток неизвестен или слота нет — ничего не меняет.
    Серии убытков, day_pnl и equity не трогает — это record_pnl."""
```
...
- `check_entry_allowed` вызывается **перед каждым входом**, включая ордера DCA и grid-лестниц, без исключений. Выход из позиции — не вход (ROUTER-EXIT, 2026-09-24): `OrderRouter.place_exit_order` (или `place_order(..., is_exit=True)`) проверяется через `check_exit_allowed`. В аварии выход должен проходить, поэтому kill-switch, breaker'ы и устаревшее equity его не держат. Зато выход не может открыть или нарастить позицию: слот по instId должен быть в риск-ядре, сторона — противоположная, размер — не больше остатка (`risk_position_size`, размер ордера входа) за вычетом неисполненных выходов роутера. Контракты выходят с `reduceOnly`, спот — с проверкой займа по режиму аккаунта. Слот освобождается по исполнению (`release_position`), а не по выставлению; PnL сделки стратегия, как и раньше, передаёт в `record_pnl`. Входы движка и грида без размера и DCA-позиции (`register_spot_buy` удаляет слот) через роутер не закрыть — отказ в безопасную сторону.
