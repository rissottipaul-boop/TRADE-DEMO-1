...
本 skill 与 `okx-maker-entry` 共用同一份 `~/.okx/config.toml`，如果你已经为 maker-entry 配过了，这里可以直接跳过，doctor 会直接通过。

## Step 1：生成配置模板

```bash
python "<skillDir>/scripts/okx_pair_spread.py" init
```
...
python "<skillDir>/scripts/okx_pair_spread.py" doctor \
  --profile demo \
  --longInstId BTC-USDT-SWAP \
  --shortInstId ETH-USDT-SWAP
```
...
- `canTrade: true`
- `oneWayMode: true`
- `longSymbolTradable: true`
- `shortSymbolTradable: true`

全部通过即完成首次配置，可以回到原始的开仓/平仓请求继续。
